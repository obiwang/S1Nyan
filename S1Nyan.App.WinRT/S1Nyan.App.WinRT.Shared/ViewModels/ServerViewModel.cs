    ...
    public class ServerViewModel : PropertyChangedBase
    {
        ...
        private void NotifyError()
        {
            ...
            if (closedServer != null)
            {
                ...
                _eventAggregator.Publish(new UserMessage(Messages.ReLogin, lastViewModel), Execute.OnUIThread);
            }
            else
            {
            ...
            if (!string.IsNullOrEmpty(_serverModel.Msg))
                msg = msg + "\r\n" + _serverModel.Msg;
            _eventAggregator.Publish(new UserMessage(Messages.ReLogin, msg), Execute.OnUIThread);
        }
